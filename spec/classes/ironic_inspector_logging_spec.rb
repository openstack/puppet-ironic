...
describe 'ironic::inspector::logging' do
  ...
  shared_examples 'basic default logging settings' do
    it 'configures ironic logging settins with default values' do
      is_expected.to contain_ironic_inspector_config('DEFAULT/use_syslog').with(:value => 'false')
      is_expected.to contain_ironic_inspector_config('DEFAULT/use_stderr').with(:value => 'true')
      ...
      is_expected.to contain_ironic_inspector_config('DEFAULT/debug').with(:value => 'false')
    end
  end
  ...
  shared_examples_for 'logging params set' do
    it 'enables logging params' do
      is_expected.to contain_ironic_inspector_config('DEFAULT/logging_context_format_string').with_value(
        '%(asctime)s.%(msecs)03d %(process)d %(levelname)s %(name)s [%(request_id)s %(user_identity)s] %(instance)s%(message)s')

      is_expected.to contain_ironic_inspector_config('DEFAULT/logging_default_format_string').with_value(
        '%(asctime)s.%(msecs)03d %(process)d %(levelname)s %(name)s [-] %(instance)s%(message)s')

      is_expected.to contain_ironic_inspector_config('DEFAULT/logging_debug_format_suffix').with_value(
        '%(funcName)s %(pathname)s:%(lineno)d')

      is_expected.to contain_ironic_inspector_config('DEFAULT/logging_exception_prefix').with_value(
       '%(asctime)s.%(msecs)03d %(process)d TRACE %(name)s %(instance)s')

      is_expected.to contain_ironic_inspector_config('DEFAULT/log_config_append').with_value(
        '/etc/ironic-inspector/logging.conf')
      is_expected.to contain_ironic_inspector_config('DEFAULT/publish_errors').with_value(
        true)

      is_expected.to contain_ironic_inspector_config('DEFAULT/default_log_levels').with_value(
      ...
      is_expected.to contain_ironic_inspector_config('DEFAULT/fatal_deprecations').with_value(
        true)

      is_expected.to contain_ironic_inspector_config('DEFAULT/instance_format').with_value(
        '[instance: %(uuid)s] ')

      is_expected.to contain_ironic_inspector_config('DEFAULT/instance_uuid_format').with_value(
        '[instance: %(uuid)s] ')

      is_expected.to contain_ironic_inspector_config('DEFAULT/log_date_format').with_value(
        '%Y-%m-%d %H:%M:%S')
    end
  end
  ...
  shared_examples_for 'logging params unset' do
   [ :logging_context_format_string, :logging_default_format_string,
     ...
     :log_date_format, ].each { |param|
        it { is_expected.to contain_ironic_inspector_config("DEFAULT/#{param}").with_ensure('absent') }
      }
  end
